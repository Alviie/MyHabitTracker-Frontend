<script setup lang="ts">
import { computed } from 'vue'

interface Habit {
  id: number
  name: string
  completed?: boolean
  createdDate?: string
  lastCompletedDate?: string
}

// Props vom Dashboard
const props = defineProps<{
  habits: Habit[]
}>()

// Letzte 7 Tage berechnen
const last7Days = computed(() => {
  const days: Date[] = []
  for (let i = 6; i >= 0; i--) {
    const date = new Date()
    date.setDate(date.getDate() - i)
    days.push(date)
  }
  return days
})

const today = new Date().toISOString().split('T')[0]

// Prozent für einen Tag berechnen
const getCompletionForDate = (date: Date) => {
  const dateStr = date.toISOString().split('T')[0]

  // Heute → aktueller Status
  if (dateStr === today) {
    const completed = props.habits.filter(h => h.completed).length
    return props.habits.length > 0
      ? (completed / props.habits.length) * 100
      : 0
  }

  // Vergangenheit
  const habitsForDate = props.habits.filter(h => {
    if (!h.createdDate) return false
    const created = new Date(h.createdDate).toISOString().split('T')[0]
    return created <= dateStr
  })

  if (habitsForDate.length === 0) return 0

  const completedOnDate = habitsForDate.filter(h => h.lastCompletedDate === dateStr).length

  return (completedOnDate / habitsForDate.length) * 100
}

// Farbe abhängig von % wählen
const getColorClass = (p: number) => {
  if (p === 0) return 'bg-gray-200'
  if (p < 50) return 'bg-red-400'
  if (p < 100) return 'bg-yellow-400'
  return 'bg-green-500'
}
</script>

<template>
  <div class="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-200">

    <!-- Titel -->
    <div class="flex items-center gap-2 mb-4">
      <svg class="w-5 h-5 text-orange-600" fill="none" stroke="currentColor" stroke-width="2"
           viewBox="0 0 24 24">
        <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
        <line x1="16" y1="2" x2="16" y2="6" />
        <line x1="8" y1="2" x2="8" y2="6" />
        <line x1="3" y1="10" x2="21" y2="10" />
      </svg>
      <h3 class="text-gray-900 font-medium">7-Tage-Überblick</h3>
    </div>

    <!-- Grid mit Tagen -->
    <div class="grid grid-cols-7 gap-2">
      <div v-for="(date, i) in last7Days" :key="i" class="text-center">

        <!-- Wochentag -->
        <p class="text-xs text-gray-500 mb-2">
          {{ date.toLocaleDateString('de-DE', { weekday: 'short' }) }}
        </p>

        <!-- Farbbox -->
        <div
          :class="[
            'w-full h-12 rounded-lg flex items-center justify-center transition-all',
            getColorClass(getCompletionForDate(date)),
            date.toISOString().split('T')[0] === today
              ? 'ring-2 ring-orange-600 ring-offset-2'
              : ''
          ]"
        >
          <span class="text-xs text-white drop-shadow">
            {{ Math.round(getCompletionForDate(date)) }}%
          </span>
        </div>

        <!-- Datum -->
        <p class="text-xs text-gray-400 mt-1">
          {{ date.getDate() }}
        </p>

      </div>
    </div>

    <!-- Legende -->
    <div class="flex items-center justify-center gap-4 mt-4 pt-4 border-t border-gray-200">
      <div class="flex items-center gap-2 text-sm text-gray-600">
        <div class="w-4 h-4 rounded bg-gray-200"></div><span>0%</span>
      </div>

      <div class="flex items-center gap-2 text-sm text-gray-600">
        <div class="w-4 h-4 rounded bg-red-400"></div><span>&lt;50%</span>
      </div>

      <div class="flex items-center gap-2 text-sm text-gray-600">
        <div class="w-4 h-4 rounded bg-yellow-400"></div><span>&lt;100%</span>
      </div>

      <div class="flex items-center gap-2 text-sm text-gray-600">
        <div class="w-4 h-4 rounded bg-green-500"></div><span>100%</span>
      </div>
    </div>

  </div>
</template>
